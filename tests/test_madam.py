...
class TestMadam:
    ...
    def read_asset(self, madam, asset):
        return madam.read(asset.essence)

    def test_get_processor_returns_processor_for_readable_asset(self, madam, asset):
        processor = madam.get_processor(asset.essence)

        assert processor is not None

    def test_get_processor_returns_none_for_unreadable_asset(self, madam, unknown_asset):
        processor = madam.get_processor(unknown_asset.essence)

        assert processor is None

    def test_read_returns_jpeg_asset_with_correct_metadata(self, madam, jpeg_data_with_exif):
    ...
    def test_read_returns_jpeg_asset_whose_essence_does_not_contain_metadata(self, madam, jpeg_image_asset, tmpdir):
    ...
    def test_read_empty_file_raises_error(self, madam):
        file_data = io.BytesIO()

        with pytest.raises(UnsupportedFormatError):
            madam.read(file_data)

    def test_read_raises_when_file_is_none(self, madam):
        invalid_file = None

        with pytest.raises(TypeError):
            madam.read(invalid_file)

    def test_read_raises_error_when_format_is_unknown(self, madam, unknown_asset):
        with pytest.raises(UnsupportedFormatError):
    ...
    def test_read_returns_asset_with_correct_mime_type(self, madam, asset):
        read_asset = madam.read(asset.essence)

        assert read_asset.mime_type == asset.mime_type

    def test_read_stores_additional_metadata(self, madam, asset):
        filename = 'foobar'
    ...
    def test_read_returns_image_asset_with_correct_color_mode(self, madam, image_asset):
        asset = image_asset
    ...
    def test_read_jpeg_does_not_alter_the_original_file(self, madam):
    ...
    def test_read_video_returns_asset_with_duration_metadata(self, madam, video_asset):
        asset = madam.read(video_asset.essence)

        assert asset.duration == pytest.approx(video_asset.duration, rel=0.4)

    def test_read_video_returns_asset_containing_video_size_metadata(self, madam, video_asset):
        asset = madam.read(video_asset.essence)

        assert asset.width == DEFAULT_WIDTH
        assert asset.height == DEFAULT_HEIGHT

    def test_read_returns_video_asset_containing_video_stream_metadata(self, madam, video_asset):
        asset = video_asset
    ...
    def test_read_returns_video_asset_containing_color_mode_metadata(self, madam, video_asset):
        asset = video_asset
    ...
    def test_read_returns_asset_containing_image_size_metadata(self, madam, image_asset):
    ...
    def test_read_returns_image_asset_containing_color_depth_metadata(self, madam, image_asset):
        image_data = image_asset.essence
    ...
    def test_read_return_correct_hierarchy_of_metadata(self, madam, jpeg_image_asset, tmpdir):
    ...
    def test_read_only_returns_python_types_in_metadata(self, madam, jpeg_image_asset, tmpdir):
    ...
    def test_writes_correct_essence_without_metadata(self, madam, asset):
    ...
    def test_writes_correct_essence_with_metadata(self, madam, jpeg_image_asset):
        file = io.BytesIO()
    ...
    def test_config_contains_list_of_all_processors_by_default(self, madam):
    ...
    def test_config_contains_list_of_all_metadata_processors_by_default(self, madam):
